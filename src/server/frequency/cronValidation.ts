...
// @ts-nocheck
// https://www.npmjs.com/package/cron-quartz-validator
...
export class CronQuartz {
  ...
  private static ERROR = {
    DAY_OF_WEEK: {
      ...
      SYNTAX:
        "Day-of-Week values must be , MON, TUE, WED, THU, FRI, SAT OR between 1 and 7, - * ? / L #",
      SYNTAX2:
  ...
  constructor(expression?: string, options?: { verbose: boolean }) {
  ...
  testMonth(expression: string) {
    ...
    if (expression.includes(",")) {
      ...
      if (values.every((v) => DAYS_OF_WEEK.includes(v.toLocaleLowerCase())))
        return true;
