...
import { iconButtonSx } from "../utils";
...
const AppNavigationMobile = (props: Props) => {
  ...
  return (
    <Box className={roboto.className} sx={{ mt: DRAWER_HEIGHT }}>
      <Box sx={{ flexGrow: 1 }}>
        <AppBar position="fixed">
          <Toolbar>
            <Box sx={{ flexGrow: 1, flexDirection: "row", ml: 1 }}>
              <DrawerHeader>
                <IconButton
                  onClick={handleToggle}
                  sx={iconButtonSx({ disabled: false, color: grey[900] })}
                >
                  {<MenuIcon />}
                </IconButton>
              </DrawerHeader>
      ...
      <Drawer
        ...
        open={open}
        sx={{
          width: open ? DRAWER_WIDTH_OPEN : 0,
          flexShrink: 0,
          "& .MuiDrawer-paper": {
            width: open ? DRAWER_WIDTH_OPEN : 0,
            boxSizing: "border-box",
